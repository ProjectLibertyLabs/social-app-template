...
export class ContentController extends BaseController {
  ...
  public async getFeed(req: Request, res: Response) {
    ...
    try {
      ...
      const response = await ContentService.getUserFeed(msaId, { newestBlockNumber, oldestBlockNumber });
      ...
      return res.status(HttpStatusCode.InternalServerError).send(err.message || 'Caught error getting feed').end();
    }
  ...
  public async getDiscover(req: Request, res: Response) {
    ...
    try {
      const oldestBlockNumber = startStr && typeof startStr === 'string' ? parseInt(startStr) : undefined;
      const newestBlockNumber = endStr && typeof endStr === 'string' ? parseInt(endStr) : undefined;
      const response = await ContentService.getDiscover({ newestBlockNumber, oldestBlockNumber });
      ...
      console.error('Error: unable to discover content: ', err);
      if (err instanceof HttpError) {
        return res.status(err.code).send(err.message || 'Caught error getting discovered content').end();
      }
