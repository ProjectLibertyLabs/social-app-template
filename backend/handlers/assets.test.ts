...
describe('POST /v2/assets/', () => {
  it('returns 202 with matched operation', async () => {
    ...
    const res = await request(app).post('/v2/assets').attach('files', Buffer.from('hello world', 'utf8'), 'file1.txt');
  ...
  it('returns 400 when missing fields property', async () => {
    const res = await request(app).post('/v2/assets').attach('file', Buffer.from('hello world', 'utf8'), 'file1.txt');
  ...
  it('returns 400 when invalid content-type', async () => {
    ...
    const res = await request(app).post('/v2/assets').set('Content-type', 'multipart/form-data');
