...
const ENV_SCHEMA = Joi.object({
    CHAIN_ENVIRONMENT: Joi.string().valid(...['dev','rococo','testnet','mainnet']).required(),
    DEBUG: Joi.bool(),
    IPFS_ENDPOINT: Joi.string().uri().required(),
    IPFS_BASIC_AUTH_USER: Joi.string(),
    IPFS_BASIC_AUTH_SECRET: Joi.string(),
    IPFS_GATEWAY_URL: Joi.string().pattern(/\[CID\]/).required().custom((value, helpers) => {
    ...
    PROVIDER_ID: Joi.number().required().positive().unsafe(true).custom((value) => BigInt(value)),
    PROVIDER_ACCOUNT_SEED_PHRASE: Joi.string()
      ...
      .custom((value: string, helpers) => {
        ...
        if (!mnemonicValidate(value)) {
          return helpers.error('any.custom', { error: new Error(`the provided value is not a valid BIP39 seed phrase`) });
        }
        return value;
      }),
...
export class Config {
  ...
  constructor(environment: Record<string, any>) {
    const { error, value } = ENV_SCHEMA.validate(environment, { abortEarly: false, allowUnknown: true, stripUnknown: true });
    if (error) {
